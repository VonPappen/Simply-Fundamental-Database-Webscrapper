...
#         print(f"No data on database for {ticker}")

def last_period_db(tickers: tuple):

    r = engine.execute(f"""SELECT  ticker, MIN(date) FROM 
    (SELECT MAX(date) as date, ticker as ticker, statement as statement
        FROM public.statements_list_table
        WHERE ticker IN {tickers}
        GROUP BY ticker, statement)
    AS SUBQUERY
    GROUP BY ticker""")

    return {i[0]:convert_to_ending_period_format(i[1]) for i in r}
...
if tickers is not None:
    ...
    df_earnings['last_period_DB'] = df_earnings['ticker'].map(last_period_db(tuple(df_earnings['ticker'].values)))
